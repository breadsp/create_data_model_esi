...
if not st.session_state.get("db_connected", False):
    ...
    st.markdown("""
        <style>
        /* Alternatively, target all vertical block gaps */
    ...
    st.markdown("<h1 style='text-align:center;font-size:28px;'>🧠 Memgraph AI Query Interface</h1>",
        unsafe_allow_html=True,
    )
...
if not st.session_state.get("model_configured", False):
    ...
    with key_cols[1]:
        if len(st.session_state.api_key_inputs) < 2:
            ...
            if st.button("Add API Key", icon=":material/add:", key="add_api_key", use_container_width=True):
    ...
    if len(st.session_state.api_key_inputs) > 1:
        ...
        with key2_cols[1]:
            st.markdown("""
                <style>
                .st-key-remove_api_key button p {
                    background-color: #FFFFFF;  #white background
                    font-size: 15px;
                    color: black;
                }
                ...
                """, unsafe_allow_html=True)
            if st.button("Remove API Key", icon=":material/remove:", key="remove_api_key", use_container_width=True):
    ...
    if st.session_state.model_option_ids:
        ...
        with btn_cols[0]:
            if st.button("", icon=":material/add:", key="add_generator_slot", use_container_width=True):
                if st.session_state.generator_slots < 5:
                    st.session_state.generator_slots += 1
                    st.session_state.generator_selected.append("")
                    st.rerun()
        with btn_cols[1]:
            if st.button("", icon=":material/remove:", key="remove_generator_slot", use_container_width=True):
...
#        summary_tab = st.tabs(["Results"])[0]
        ...
        if submit_button and user_query:
            ...
            judge_timing_rows = []
            try:
                print("the models are running to create the cypher")
                st.session_state.model_runs = run_generation_models(user_query)
                print("the judge is running to interpert the model results")
                ...
                if selected_cypher:
                    print("the cypher is being validated for syntax")
                ...
                if working_cypher:
                    for attempt in range(1, MAX_REPAIR_ATTEMPTS + 1):
                        ...
                        if ok:
                            ...
                            break
                        print("the cypher is being repaired if necessary")
    ...
    with summary_tab:
        ...
        if st.session_state.cypher_str != "":
            try:
                ...
                # ─────────────────────────────────────────────────────────────
                st.session_state.mem_db.cursor.execute(query)
                print("the Cypher is being exectued and results are being returned")
                result = st.session_state.mem_db.cursor.fetchall()
